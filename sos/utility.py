...
def Exit(message: 'str'="", code: 'int'=1, excp: 'Any'=None):  # line 236
    if not '--quiet' in sys.argv:  # line 237
    ...
    if '--wait' in sys.argv:  # line 246
        input("Hit Enter to finish." if not '--quiet' in sys.argv else "")  # line 246
    sys.exit(code)  # line 247

def fitStrings(strings: '_coconut.typing.Sequence[str]', prefix: 'str', length: 'int'=MAX_COMMAND_LINE.get(sys.platform, MAX_COMMAND_LINE[None]), separator: 'str'=" ", process: '_coconut.typing.Callable[..., str]'=lambda _=None: '"%s"' % _) -> 'str':  # line 249
    ''' Returns a packed string, destructively consuming entries from the provided list. Does similar as xargs. getconf ARG_MAX or xargs --show-limits. '''  # line 250
    if len(prefix + separator + ((process)(strings[0]))) > length:  # line 251
        raise Exception("Cannot possibly strings pack into specified length")  # line 251
    while len(strings) > 0 and len(prefix + separator + ((process)(strings[0]))) <= length:  # line 252
        prefix += separator + ((process)(strings.pop(0)))  # line 252
    return prefix  # line 253

def exception(E) -> 'str':  # line 255
    ''' Report an exception to the user to allow useful bug reporting. '''  # line 256
    import traceback  # line 257
    return str(E) + "\n" + traceback.format_exc() + "\n" + traceback.format_list(traceback.extract_stack())  # line 258

def hashFile(path: 'str', compress: 'bool', saveTo: 'List[str]'=[], callback: 'Optional[_coconut.typing.Callable[[str], None]]'=None, symbols: 'str'=PROGRESS_MARKER[0]) -> 'Tuple[str, int]':  # line 260
    ''' Calculate and return (hash of file contents, compressed sized (if writing) else 0). '''  # line 261
    indicator = ProgressIndicator(symbols, callback) if callback else None  # type: _coconut.typing.Optional[ProgressIndicator]  # line 262
    _hash = hashlib.sha256()  # line 263
    wsize = 0  # type: int  # line 264
    if saveTo and os.path.exists(encode(saveTo[0])):  # line 265
        Exit("Hash collision detected. Leaving repository in inconsistent state", 1)  # HINT this exits immediately  # line 266
    to = openIt(saveTo[0], "w", compress) if saveTo else None  # line 267
    retry = RETRY_NUM  # type: int  # line 268
    while True:  # line 269
        try:  # line 270
            ...
            break  # line 282
        except Exception as E:  # (IsADirectoryError, PermissionError)  # line 283
            retry -= 1  # line 284
            if retry == 0:  # line 285
                raise E  # line 285
            error("Cannot open %r - retrying %d more times in %.1d seconds" % (path, RETRY_WAIT))  # line 286
            time.sleep(RETRY_WAIT)  # line 287
    return (_hash.hexdigest(), wsize)  # line 288

def getAnyOfMap(map: 'Dict[str, Any]', params: '_coconut.typing.Sequence[str]', default: 'Any'=None) -> 'Any':  # line 290
    ''' Utility to find any entries of a dictionary in a list to return the dictionaries value. '''  # line 291
    for k, v in map.items():  # line 292
        if k in params:  # line 292
            return v  # line 292
    return default  # line 293

@_coconut_tco  # line 295
def strftime(timestamp: '_coconut.typing.Optional[int]'=None) -> 'str':  # line 295
    return _coconut_tail_call(time.strftime, "%Y-%m-%d %H:%M:%S", time.localtime(timestamp / 1000. if timestamp is not None else None))  # line 295

def detectAndLoad(filename: '_coconut.typing.Optional[str]'=None, content: '_coconut.typing.Optional[bytes]'=None, ignoreWhitespace: 'bool'=False) -> 'Tuple[str, _coconut.typing.Optional[bytes], _coconut.typing.Sequence[str]]':  # line 297
...
def mergeClassic(file: 'bytes', intofile: 'str', fromname: 'str', intoname: 'str', totimestamp: 'int', context: 'int', ignoreWhitespace: 'bool'=False):  # line 351
    ...
    except Exception as E:  # in case of binary files  # line 356
        Exit("Cannot diff '%s' vs '%s': %r" % (("<bytes>" if fromname is None else fromname), ("<bytes>" if intoname is None else intoname)), excp=E)  # in case of binary files  # line 356
...
def merge(file: '_coconut.typing.Optional[bytes]'=None, into: '_coconut.typing.Optional[bytes]'=None, filename: '_coconut.typing.Optional[str]'=None, intoname: '_coconut.typing.Optional[str]'=None, mergeOperation: 'MergeOperation'=MergeOperation.BOTH, charMergeOperation: 'MergeOperation'=MergeOperation.BOTH, diffOnly: 'bool'=False, eol: 'bool'=False, ignoreWhitespace: 'bool'=False) -> 'Tuple[Union[bytes, List[MergeBlock]], _coconut.typing.Optional[bytes]]':  # line 359
  ...
  '''  # line 374
    ...
    try:  # load files line-wise and normalize line endings (keep the one of the current file) TODO document  # line 376
        ...
        Exit("Cannot merge '%s' into '%s': %r" % (("<bytes>" if filename is None else filename), ("<bytes>" if intoname is None else intoname)), excp=E)  # line 379
...
#      elif mergeOperation == MergeOperation.ASK:  # more than one line: needs user input
            if mergeOperation == MergeOperation.ASK:  # more than one line: needs user input  # line 421
                printo(pure.ajoin("- ", block.lines, nl="\n"))  # TODO check +/- in update mode, could be swapped  # line 422
                printo(pure.ajoin("+ ", block.replaces.lines, nl="\n"))  # line 423
                while True:  # line 424
                    op = input(" Line replacement: *M[I]ne (+), [T]heirs (-), [B]oth, [U]ser input: ").strip().lower()[:1]  # type: str  # line 425
                    if op in "tb":  # line 426
                        output.extend(block.lines)  # line 426
                    if op in "ib":  # line 427
                        output.extend(block.replaces.lines)  # line 427
                    if op == "u":  # line 428
                        user_block_input(output)  # line 428
                    if op in "tbiu":  # line 429
                        break  # line 429
            else:  # more than one line and not ask  # line 430
                if mergeOperation == MergeOperation.REMOVE:  # line 431
                    pass  # line 431
                elif mergeOperation == MergeOperation.BOTH:  # line 432
                    output.extend(block.lines)  # line 432
                elif mergeOperation == MergeOperation.INSERT:  # TODO optionally allow insertion BEFORE or AFTER original (order of these both lines)  # line 433
                    output.extend(list(block.replaces.lines) + list(block.lines))  # TODO optionally allow insertion BEFORE or AFTER original (order of these both lines)  # line 433
        elif block.tipe in (MergeBlockType.INSERT, MergeBlockType.REMOVE) and mergeOperation == MergeOperation.ASK:  # user - interactive insert/remove section  # line 434
            if (block.tipe == MergeBlockType.INSERT and add_all is None) or (block.tipe == MergeOperation.REMOVE and del_all is None):  # condition for asking  # line 435
                selection = user_input(pure.ajoin("+ " if block.tipe == MergeBlockType.INSERT else "- ", block.lines) + "\n  Accept? *[Y]es, [N]o, yes to [A]ll %s, n[O] to all: " % "insertions" if block.tipe == MergeBlockType.INSERT else "deletions", "ynao", "y")  # line 437
                if selection in "ao":  # line 438
                    if block.tipe == MergeBlockType.INSERT:  # line 439
                        add_all = "y" if selection == "a" else "n"  # line 439
                        selection = add_all  # line 439
                    else:  # REMOVE case  # line 440
                        del_all = "y" if selection == "a" else "n"  # REMOVE case  # line 440
                        selection = del_all  # REMOVE case  # line 440
            if (block.tipe == MergeBlockType.INSERT and "y" in (add_all, selection)) or ("n" in (del_all, selection)):  # REMOVE case  # line 441
                output.extend(block.lines)  # line 443
    debug("Merge output: " + "\n".join(output))  # line 444
    return (((b"\n" if nl is None else nl)).join([line.encode(encoding) for line in output]), nl)  # returning bytes  # line 445
# TODO handle check for more/less lines in found -/+ blocks to find common section and splitting prefix/suffix out

@_coconut_tco  # line 448
def lineMerge(othr: 'str', into: 'str', mergeOperation: 'MergeOperation'=MergeOperation.BOTH, diffOnly: 'bool'=False) -> 'Union[str, List[MergeBlock]]':  # line 448
    ''' Merges string 'othr' into current string 'into'.
      change direction mark is insert for elements only in into, and remove for elements only in file (according to diff marks +/-)
  '''  # line 451
    out = list(difflib.Differ().compare(othr, into))  # type: List[str]  # line 452
    blocks = []  # type: List[MergeBlock]  # line 453
    for i, line in enumerate(out):  # line 454
        if line[0] == "+":  # line 455
            if i + 1 < len(out) and out[i + 1][0] == "+":  # block will continue  # line 456
                if i > 0 and blocks[-1].tipe == MergeBlockType.INSERT:  # middle of + block  # line 457
                    blocks[-1].lines.append(line[2])  # add one more character to the accumulating list  # line 458
                else:  # first + in block  # line 459
                    blocks.append(MergeBlock(MergeBlockType.INSERT, [line[2]], i))  # line 460
            else:  # last line of + block  # line 461
                if i > 0 and blocks[-1].tipe == MergeBlockType.INSERT:  # end of a block  # line 462
                    blocks[-1].lines.append(line[2])  # line 463
                else:  # single line  # line 464
                    blocks.append(MergeBlock(MergeBlockType.INSERT, [line[2]], i))  # line 465
                if i >= 1 and blocks[-2].tipe == MergeBlockType.REMOVE:  # previous - and now last in + block creates a replacement block  # line 466
                    blocks[-2] = MergeBlock(MergeBlockType.REPLACE, blocks[-2].lines, i, replaces=blocks[-1])  # line 467
                    blocks.pop()  # line 467
        elif line[0] == "-":  # line 468
            if i > 0 and blocks[-1].tipe == MergeBlockType.REMOVE:  # part of - block  # line 469
                blocks[-1].lines.append(line[2])  # line 470
            else:  # first in block  # line 471
                blocks.append(MergeBlock(MergeBlockType.REMOVE, [line[2]], i))  # line 472
        elif line[0] == " ":  # line 473
            if i > 0 and blocks[-1].tipe == MergeBlockType.KEEP:  # part of block  # line 474
                blocks[-1].lines.append(line[2])  # line 475
            else:  # first in block  # line 476
                blocks.append(MergeBlock(MergeBlockType.KEEP, [line[2]], i))  # line 477
        else:  # line 478
            raise Exception("Cannot parse diff line %r" % line)  # line 478
    blocks[:] = [dataCopy(MergeBlock, block, lines=["".join(block.lines)], replaces=dataCopy(MergeBlock, block.replaces, lines=["".join(block.replaces.lines)]) if block.replaces else None) for block in blocks]  # line 479
    if diffOnly:  # line 480
        return blocks  # line 480
    out[:] = []  # line 481
    for i, block in enumerate(blocks):  # line 482
        if block.tipe == MergeBlockType.KEEP:  # line 483
            out.extend(block.lines)  # line 483
        elif block.tipe == MergeBlockType.REPLACE:  # line 484
            if mergeOperation == MergeOperation.ASK:  # line 485
                printo(pure.ajoin("- ", othr))  # line 486
                printo("- " + (" " * i) + block.replaces.lines[0])  # line 487
                printo("+ " + (" " * i) + block.lines[0])  # line 488
                printo(pure.ajoin("+ ", into))  # line 489
                op = user_input(" Character replacement: *M[I]ne (+), [T]heirs (-), [B]oth, [U]ser input: ", "tbim")  # type: str  # line 490
                if op in "tb":  # line 491
                    out.extend(block.lines)  # line 491
                    break  # line 491
                if op in "ib":  # line 492
                    out.extend(block.replaces.lines)  # line 492
                    break  # line 492
                if op == "m":  # line 493
                    user_block_input(out)  # line 493
                    break  # line 493
            else:  # non-interactive  # line 494
                if mergeOperation == MergeOperation.REMOVE:  # line 495
                    pass  # line 495
                elif mergeOperation == MergeOperation.BOTH:  # line 496
                    out.extend(block.lines)  # line 496
                elif mergeOperation == MergeOperation.INSERT:  # line 497
                    out.extend(list(block.replaces.lines) + list(block.lines))  # line 497
        elif block.tipe == MergeBlockType.INSERT and not (mergeOperation.value & MergeOperation.REMOVE.value):  # line 498
            out.extend(block.lines)  # line 498
        elif block.tipe == MergeBlockType.REMOVE and mergeOperation.value & MergeOperation.INSERT.value:  # line 499
            out.extend(block.lines)  # line 499
# TODO ask for insert or remove as well
    return _coconut_tail_call("".join, out)  # line 501

def findSosVcsBase() -> 'Tuple[_coconut.typing.Optional[str], _coconut.typing.Optional[str], _coconut.typing.Optional[str]]':  # line 503
    ''' Attempts to find sos and legacy VCS base folders.
      Returns (SOS-repo root, VCS-repo root, VCS command)
  '''  # line 506
    debug("Detecting root folders...")  # line 507
    path = os.getcwd()  # type: str  # start in current folder, check parent until found or stopped  # line 508
    vcs = (None, None)  # type: Tuple[_coconut.typing.Optional[str], _coconut.typing.Optional[str]]  # line 509
    while not os.path.exists(encode(os.path.join(path, metaFolder))):  # line 510
        contents = set(os.listdir(path))  # type: Set[str]  # line 511
        vcss = [executable for folder, executable in vcsFolders.items() if folder in contents or (SLASH in folder and os.path.exists(os.path.join(os.getcwd(), folder.replace(SLASH, os.sep))))]  # type: _coconut.typing.Sequence[str]  # determine VCS type from existence of dot folder TODO use encode?  # line 512
        choice = None  # type: _coconut.typing.Optional[str]  # line 513
        if len(vcss) > 1:  # line 514
            choice = SVN if SVN in vcss else vcss[0]  # SVN is preferred  # line 515
        ...
        if not vcs[0] and choice:  # memorize current repo root  # line 518
            vcs = (path, choice)  # memorize current repo root  # line 518
        new = os.path.dirname(path)  # get parent path  # line 519
        if new == path:  # avoid infinite loop  # line 520
            break  # avoid infinite loop  # line 520
        path = new  # line 521
    if os.path.exists(encode(os.path.join(path, metaFolder))):  # found something  # line 522
        if vcs[0]:  # already detected vcs base and command  # line 523
            return (path, vcs[0], vcs[1])  # already detected vcs base and command  # line 523
        sos = path  # line 524
        while True:  # continue search for VCS base  # line 525
            contents = set(os.listdir(path))  # line 526
            vcss = [executable for folder, executable in vcsFolders.items() if folder in contents]  # determine VCS type  # line 527
            choice = None  # line 528
            if len(vcss) > 1:  # line 529
                choice = SVN if SVN in vcss else vcss[0]  # line 530
                warn("Detected more than one parallel VCS checkouts %r. Falling back to '%s'" % (vcss, choice))  # line 531
            elif len(vcss) > 0:  # line 532
                choice = vcss[0]  # line 532
            if choice:  # line 533
                return (sos, path, choice)  # line 533
            new = os.path.dirname(path)  # get parent path  # line 534
            if new == path:  # no VCS folder found  # line 535
                return (sos, None, None)  # no VCS folder found  # line 535
            path = new  # line 536
    return (None, vcs[0], vcs[1])  # line 537

def tokenizeGlobPattern(pattern: 'str') -> 'List[GlobBlock]':  # line 539
    index = 0  # type: int  # line 540
    out = []  # type: List[GlobBlock]  # literal = True, first index  # line 541
    while index < len(pattern):  # line 542
        if pattern[index:index + 3] in ("[?]", "[*]", "[[]", "[]]"):  # line 543
            out.append(GlobBlock(False, pattern[index:index + 3], index))  # line 543
            continue  # line 543
        if pattern[index] in "*?":  # line 544
            count = 1  # type: int  # line 545
            while index + count < len(pattern) and pattern[index] == "?" and pattern[index + count] == "?":  # line 546
                count += 1  # line 546
            out.append(GlobBlock(False, pattern[index:index + count], index))  # line 547
            index += count  # line 547
            continue  # line 547
        if pattern[index:index + 2] == "[!":  # line 548
            out.append(GlobBlock(False, pattern[index:pattern.index("]", index + 2) + 1], index))  # line 548
            index += len(out[-1][1])  # line 548
            continue  # line 548
        count = 1  # line 549
        while index + count < len(pattern) and pattern[index + count] not in "*?[":  # line 550
            count += 1  # line 550
        out.append(GlobBlock(True, pattern[index:index + count], index))  # line 551
        index += count  # line 551
    return out  # line 552

def tokenizeGlobPatterns(oldPattern: 'str', newPattern: 'str') -> 'Tuple[_coconut.typing.Sequence[GlobBlock], _coconut.typing.Sequence[GlobBlock]]':  # line 554
    ot = tokenizeGlobPattern(oldPattern)  # type: List[GlobBlock]  # line 555
    nt = tokenizeGlobPattern(newPattern)  # type: List[GlobBlock]  # line 556
#  if len(ot) != len(nt): Exit("Source and target patterns can't be translated due to differing number of parsed glob markers and literal strings")
    if len([o for o in ot if not o.isLiteral]) < len([n for n in nt if not n.isLiteral]):  # line 558
        Exit("Source and target file patterns contain differing number of glob markers and can't be translated")  # line 558
    if any((O.content != N.content for O, N in zip([o for o in ot if not o.isLiteral], [n for n in nt if not n.isLiteral]))):  # line 559
        Exit("Source and target file patterns differ in semantics")  # line 559
    return (ot, nt)  # line 560

def convertGlobFiles(filenames: '_coconut.typing.Sequence[str]', oldPattern: '_coconut.typing.Sequence[GlobBlock]', newPattern: '_coconut.typing.Sequence[GlobBlock]') -> '_coconut.typing.Sequence[Tuple[str, str]]':  # line 562
    ''' Converts given filename according to specified file patterns. No support for adjacent glob markers currently. '''  # line 563
    pairs = []  # type: List[Tuple[str, str]]  # line 564
    for filename in filenames:  # line 565
        literals = [l for l in oldPattern if l.isLiteral]  # type: List[GlobBlock]  # source literals  # line 566
        nextliteral = 0  # type: int  # line 567
        index = 0  # type: int  # line 567
        parsedOld = []  # type: List[GlobBlock2]  # line 568
        for part in oldPattern:  # match everything in the old filename  # line 569
            if part.isLiteral:  # line 570
                parsedOld.append(GlobBlock2(True, part.content, part.content))  # line 570
                index += len(part.content)  # line 570
                nextliteral += 1  # line 570
            elif part.content.startswith("?"):  # line 571
                parsedOld.append(GlobBlock2(False, part.content, filename[index:index + len(part.content)]))  # line 571
                index += len(part.content)  # line 571
            elif part.content.startswith("["):  # line 572
                parsedOld.append(GlobBlock2(False, part.content, filename[index]))  # line 572
                index += 1  # line 572
            elif part.content == "*":  # line 573
                if nextliteral >= len(literals):  # line 574
                    parsedOld.append(GlobBlock2(False, part.content, filename[index:]))  # line 574
                    break  # line 574
                nxt = filename.index(literals[nextliteral].content, index)  # type: int  # also matches empty string  # line 575
                parsedOld.append(GlobBlock2(False, part.content, filename[index:nxt]))  # line 576
                index = nxt  # line 576
            else:  # line 577
                Exit("Invalid file pattern specified for move/rename")  # line 577
        globs = [g for g in parsedOld if not g.isLiteral]  # type: List[GlobBlock2]  # line 578
        literals = [l for l in newPattern if l.isLiteral]  # target literals  # line 579
        nextliteral = 0  # line 580
        nextglob = 0  # type: int  # line 580
        outname = []  # type: List[str]  # line 581
        for part in newPattern:  # generate new filename  # line 582
            if part.isLiteral:  # line 583
                outname.append(literals[nextliteral].content)  # line 583
                nextliteral += 1  # line 583
            else:  # line 584
                outname.append(globs[nextglob].matches)  # line 584
                nextglob += 1  # line 584
        pairs.append((filename, "".join(outname)))  # line 585
    return pairs  # line 586

@_coconut_tco  # line 588
def reorderRenameActions(actions: '_coconut.typing.Sequence[Tuple[str, str]]', exitOnConflict: 'bool'=True) -> '_coconut.typing.Sequence[Tuple[str, str]]':  # line 588
    ''' Attempt to put all rename actions into an order that avoids target == source names.
      Note, that it's currently not really possible to specify patterns that make this work (swapping "*" elements with a reference).
      An alternative would be to always have one (or all) files renamed to a temporary name before renaming to target filename.
  '''  # line 592
    if not actions:  # line 593
        return []  # line 593
    sources = None  # type: List[str]  # line 594
    targets = None  # type: List[str]  # line 594
    sources, targets = [list(l) for l in zip(*actions)]  # line 595
    last = len(actions)  # type: int  # line 596
    while last > 1:  # line 597
        clean = True  # type: bool  # line 598
        for i in range(1, last):  # line 599
            try:  # line 600
                index = targets[:i].index(sources[i])  # type: int  # line 601
                sources.insert(index, sources.pop(i))  # bubble up the action right before conflict  # line 602
                targets.insert(index, targets.pop(i))  # line 603
                clean = False  # line 604
            except:  # target not found in sources: good!  # line 605
                continue  # target not found in sources: good!  # line 605
        if clean:  # line 606
            break  # line 606
        last -= 1  # we know that the last entry in the list has the least conflicts, so we can disregard it in the next iteration  # line 607
    if exitOnConflict:  # line 608
        for i in range(1, len(actions)):  # line 608
            if sources[i] in targets[:i]:  # line 608
                Exit("There is no order of renaming actions that avoids copying over not-yet renamed files: '%s' is contained in matching source filenames" % (targets[i]))  # line 608
    return _coconut_tail_call(list, zip(sources, targets))  # convert to list to avoid generators  # line 609

def relativize(root: 'str', filepath: 'str') -> 'Tuple[str, str]':  # line 611
    ''' Determine OS-independent relative folder path, and relative pattern path. Always expects a file and determines its folder's relative path. '''  # line 612
    relpath = os.path.relpath(os.path.dirname(os.path.abspath(filepath)), root).replace(os.sep, SLASH)  # line 613
    return relpath, os.path.join(relpath, os.path.basename(filepath)).replace(os.sep, SLASH)  # line 614

def parseArgumentOptions(cwd: 'str', options: 'List[str]') -> 'Tuple[_coconut.typing.Optional[FrozenSet[str]], _coconut.typing.Optional[FrozenSet[str]], List[str], List[str]]':  # line 616
    ''' Returns (root-normalized) set of --only and --except arguments. '''  # line 617
    root = os.getcwd()  # type: str  # line 618
    onlys = []  # type: List[str]  # line 619
    excps = []  # type: List[str]  # line 619
    remotes = []  # type: List[str]  # line 619
    noremotes = []  # type: List[str]  # line 619
    for keys, container in [(("--only", "--include"), onlys), (("--except", "--exclude"), excps), (("--remote", "--remotes", "--only-remote", "--only-remotes", "--include-remote", "--include-remotes"), remotes), (("--except-remote", "--except-remotes", "--exclude-remote", "--exclude-remotes"), noremotes)]:  # line 620
        founds = [i for i in range(len(options)) if any([options[i].startswith(key + "=") or options[i] == key for key in keys])]  # assuming no more than one = in the string  # line 621
        for i in reversed(founds):  # line 622
            if "=" in options[i]:  # line 623
                container.extend(safeSplit(options[i].split("=")[1], ";"))  # TODO keep semicolon or use comma?  # line 624
            elif i + 1 < len(options):  # in case last --only has no argument  # line 625
                container.extend(safeSplit(options[i + 1], ";"))  # TODO test this  # line 626
                del options[i + 1]  # line 627
            del options[i]  # reverse removal  # line 628
    return (frozenset((oo for oo in (relativize(root, os.path.normpath(os.path.join(cwd, o)))[1] for o in onlys) if not oo.startswith(PARENT + SLASH))) if onlys else None, frozenset((ee for ee in (relativize(root, os.path.normpath(os.path.join(cwd, e)))[1] for e in excps) if not ee.startswith(PARENT + SLASH))) if excps else None, [os.path.abspath(os.path.normpath(_)) for _ in remotes], [os.path.abspath(os.path.normpath(_)) for _ in noremotes])  # line 629
