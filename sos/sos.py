...
#      branches:List[List]  # deserialized JSON is only list, while the real type of _.branches is a dict number -> BranchInfo (Coconut data type/named tuple)
    ...
    '''  # line 256
        import collections  # used almost only here  # line 257
        binfo = None  # type: BranchInfo  # typing info  # line 258
...
#  debug(('%s commit -m "%s" %s' % (cmd, message ?? ("Committed from SOS %s/r%02d on %s" % (m.branches[branch].name ?? ("b%d" % m.branch), revision, strftime())).replace("\"", "\\\""), commitArgs ?? "")))
    ...
    m.branches[m.branch] = dataCopy(BranchInfo, m.branches[m.branch], inSync=True)  # mark as committed to underlying  # line 1143
    m.saveBranches()  # line 1144

def config(arguments: 'List[_coconut.typing.Optional[str]]', options: 'List[str]'=[]):  # line 1146
    command = None  # type: str  # line 1147
    key = None  # type: str  # line 1147
    value = None  # type: str  # line 1147
    v = None  # type: str  # line 1147
    command, key, value = (arguments + [None] * 2)[:3]  # line 1148
    if command is None:  # line 1149
        usage.usage("help", verbose=True)  # line 1149
    if command not in ("set", "unset", "show", "list", "add", "rm"):  # line 1150
        Exit("Unknown config command %r" % command)  # line 1150
    local = "--local" in options  # type: bool  # line 1151
    m = Metadata()  # type: Metadata  # loads nested configuration (local - global - defaults)  # line 1152
    c = m.c if local else m.c.__defaults  # type: configr.Configr  # line 1153
    if command == "set":  # line 1154
        if None in (key, value):  # line 1155
            Exit("Key or value not specified")  # line 1155
        if key not in ((([] if local else ONLY_GLOBAL_FLAGS) + CONFIGURABLE_FLAGS + ["defaultbranch"]) + CONFIGURABLE_LISTS + CONFIGURABLE_INTS):  # TODO move defaultbranch to configurable_texts?  # line 1156
            Exit("Unsupported key for %s configuration %r" % ("local" if local else "global", key))  # TODO move defaultbranch to configurable_texts?  # line 1156
        if key in (ONLY_GLOBAL_FLAGS + CONFIGURABLE_FLAGS) and value.lower() not in TRUTH_VALUES + FALSE_VALUES:  # line 1157
            Exit("Cannot set flag to '%s'. Try on/off instead" % value.lower())  # line 1157
        c[key] = value.lower() in TRUTH_VALUES if key in (ONLY_GLOBAL_FLAGS + CONFIGURABLE_FLAGS) else (tryOrIgnore(lambda _=None: int(value), lambda E: error("Not an integer value: %r" % E)) if key in CONFIGURABLE_INTS else (removePath(key, value.strip()) if key not in CONFIGURABLE_LISTS else [removePath(key, v) for v in safeSplit(value, ";")]))  # TODO sanitize texts?  # line 1158
    elif command == "unset":  # line 1159
        if key is None:  # line 1160
            Exit("No key specified")  # line 1160
        if key not in c.keys(with_nested=False):  # line 1161
            Exit(("Unknown key %r" % key) if not key in c.keys(with_nested=local, with_defaults=True) else "Key %r not defined in %s scope" % (key, "local" if local else "global"))  # line 1162
        del c[key]  # line 1163
    elif command == "add":  # TODO copy list from defaults if not local/global  # line 1164
        if None in (key, value):  # line 1165
            Exit("Key or value not specified")  # line 1165
        if key not in CONFIGURABLE_LISTS:  # line 1166
            Exit("Unsupported key %r for list addition" % key)  # line 1166
        if key not in c.keys():  # prepare empty list, or copy from underlying, add new value below TODO also allow one more level of underlying?  # line 1167
            c[key] = [_ for _ in c.__defaults[key]] if key in c.__defaults[key] else []  # prepare empty list, or copy from underlying, add new value below TODO also allow one more level of underlying?  # line 1167
        elif value in c[key]:  # line 1168
            Exit("Value already contained, nothing to do")  # line 1168
        if ";" not in value:  # line 1169
            c[key].append(removePath(key, value.strip()))  # line 1169
        else:  # line 1170
            c[key].extend([removePath(key, v) for v in safeSplit(value, ";")])  # line 1170
    elif command == "rm":  # line 1171
        if None in (key, value):  # line 1172
            Exit("Key or value not specified")  # line 1172
        if key not in c.keys(with_nested=False):  # line 1173
            Exit(("Unknown key %r" % key) if not key in c.keys(with_nested=local, with_defaults=True) else "Key %r not defined in %s scope" % (key, "local" if local else "global"))  # line 1174
        if value not in c[key]:  # line 1175
            Exit("Unknown value %r" % value)  # line 1175
        c[key].remove(value)  # line 1176
        if local and len(c[key]) == 0 and "--prune" in options:  # remove local entry, to fallback to global  # line 1177
            del c[key]  # remove local entry, to fallback to global  # line 1177
    else:  # Show or list  # line 1178
        if key == "ints":  # list valid configuration items  # line 1179
            printo(", ".join(CONFIGURABLE_INTS))  # list valid configuration items  # line 1179
        elif key == "flags":  # line 1180
            printo(", ".join(ONLY_GLOBAL_FLAGS + CONFIGURABLE_FLAGS))  # line 1180
        elif key == "lists":  # line 1181
            printo(", ".join(CONFIGURABLE_LISTS))  # line 1181
        elif key == "texts":  # line 1182
            printo(", ".join([_ for _ in defaults.keys() if _ not in (ONLY_GLOBAL_FLAGS + CONFIGURABLE_FLAGS + CONFIGURABLE_INTS + CONFIGURABLE_LISTS)]))  # line 1182
        else:  # no key: list all  # line 1183
            out = {3: "[default]", 2: "[global] ", 1: "[local]  "}  # type: Dict[int, str]  # in contrast to Git, we don't need (nor want) to support a "system" config scope  # line 1184
            c = m.c  # always use full configuration chain  # line 1185
            try:  # attempt single key  # line 1186
                assert key is not None  # force exception if no key specified  # line 1187
                c[key]  # force exception if no key specified  # line 1187
                l = key in c.keys(with_nested=False)  # type: bool  # line 1188
                g = key in c.__defaults.keys(with_nested=False)  # type: bool  # line 1188
                printo(key.rjust(20), color=Fore.WHITE, nl="")  # line 1189
                printo(" " + (out[3] if not (l or g) else (out[1] if l else out[2])) + " ", color=Fore.CYAN, nl="")  # line 1190
                printo(repr(c[key]))  # line 1191
            except:  # normal value listing  # line 1192
                vals = {k: (repr(v), 3) for k, v in defaults.items()}  # type: Dict[str, Tuple[str, int]]  # copy-by-value  # line 1193
                vals.update({k: (repr(v), 2) for k, v in c.__defaults.items()})  # line 1194
                vals.update({k: (repr(v), 1) for k, v in c.__map.items()})  # line 1195
                for k, vt in sorted(vals.items()):  # line 1196
                    printo(k.rjust(20), color=Fore.WHITE, nl="")  # line 1197
                    printo(" " + out[vt[1]] + " ", color=Fore.CYAN, nl="")  # line 1198
                    printo(vt[0])  # line 1199
                if len(c.keys()) == 0:  # line 1200
                    info("No local configuration stored.")  # line 1200
                if len(c.__defaults.keys()) == 0:  # line 1201
                    info("No global configuration stored.")  # line 1201
        return  # in case of list, no need to store anything  # line 1202
    if local:  # saves changes of repoConfig  # line 1203
        m.repoConf = c.__map  # saves changes of repoConfig  # line 1203
        m.saveBranches(m._extractRemotesFromArguments(options))  # saves changes of repoConfig  # line 1203
        Exit("OK", code=0)  # saves changes of repoConfig  # line 1203
    else:  # global config  # line 1204
        f, h = saveConfig(c)  # only saves c.__defaults (nested Configr)  # line 1205
        if f is None:  # line 1206
            Exit("Error saving user configuration: %r" % h)  # line 1206

def move(relPath: 'str', pattern: 'str', newRelPath: 'str', newPattern: 'str', options: 'List[str]'=[], negative: 'bool'=False):  # line 1208
    ''' Path differs: Move files, create folder if not existing. Pattern differs: Attempt to rename file, unless exists in target or not unique.
      for "mvnot" don't do any renaming (or do?)
  '''  # line 1211
    if verbose:  # line 1212
        info(MARKER + "Renaming %r to %r" % (pattern, newPattern))  # line 1212
    force = '--force' in options  # type: bool  # line 1213
    soft = '--soft' in options  # type: bool  # line 1214
    if not os.path.exists(encode(relPath.replace(SLASH, os.sep))) and not force:  # line 1215
        Exit("Source folder doesn't exist. Use --force to proceed anyway")  # line 1215
    m = Metadata()  # type: Metadata  # line 1216
    patterns = m.branches[m.branch].untracked if negative else m.branches[m.branch].tracked  # type: List[str]  # line 1217
    files = os.listdir(relPath.replace(SLASH, os.sep)) if os.path.exists(encode(relPath.replace(SLASH, os.sep))) else []  # type: List[str]  # line 1218
    files[:] = [f for f in files if len([n for n in m.c.ignores if fnmatch.fnmatch(f, n)]) == 0 or len([p for p in m.c.ignoresWhitelist if fnmatch.fnmatch(f, p)]) > 0]  # line 1219
    matching = fnmatch.filter(files, os.path.basename(pattern))  # type: List[str]  # find matching files in source  # line 1220
    if not matching and not force:  # line 1221
        Exit("No files match the specified file pattern. Use --force to proceed anyway")  # line 1221
    if not (m.track or m.picky):  # line 1222
        Exit("Repository is in simple mode. Use basic file operations to modify files, then execute 'sos commit' to version any changes")  # line 1222
    if pattern not in patterns:  # list potential alternatives and exit  # line 1223
        for tracked in (t for t in patterns if t[:t.rindex(SLASH)] == relPath):  # for all patterns of the same source folder HINT was os.path.dirpath before  # line 1224
            alternative = fnmatch.filter(files, os.path.basename(tracked))  # type: _coconut.typing.Sequence[str]  # find if it matches any of the files in the source folder, too  # line 1225
            if alternative:  # line 1226
                info("  '%s' matches %d file%s" % (tracked, len(alternative), "s" if len(alternative) > 1 else ""))  # line 1226
        Exit("File pattern '%s' is not tracked on current branch. 'sos move' only works on tracked patterns" % pattern)  # HINT removed: "if not (force or soft):""  # line 1227
    basePattern = os.path.basename(pattern)  # type: str  # pure glob without folder  # line 1228
    newBasePattern = os.path.basename(newPattern)  # type: str  # line 1229
    if basePattern.count("*") < newBasePattern.count("*") or (basePattern.count("?") - basePattern.count("[?]")) < (newBasePattern.count("?") - newBasePattern.count("[?]")) or (basePattern.count("[") - basePattern.count("\\[")) < (newBasePattern.count("[") - newBasePattern.count("\\[")) or (basePattern.count("]") - basePattern.count("\\]")) < (newBasePattern.count("]") - newBasePattern.count("\\]")):  # line 1230
        Exit("Glob markers from '%s' to '%s' don't match, cannot move/rename tracked matching file(s)" % (basePattern, newBasePattern))  # line 1234
#  oldTokens:GlobBlock[]?; newToken:GlobBlock[]?  # TODO remove optional?, only here to satisfy mypy
    oldTokens, newTokens = tokenizeGlobPatterns(os.path.basename(pattern), os.path.basename(newPattern))  # line 1236
    matches = convertGlobFiles(matching, oldTokens, newTokens)  # type: _coconut.typing.Sequence[Tuple[str, str]]  # computes list of source - target filename pairs  # line 1237
    if len({st[1] for st in matches}) != len(matches):  # line 1238
        Exit("Some target filenames are not unique and different move/rename actions would point to the same target file")  # line 1238
    matches = reorderRenameActions(matches, exitOnConflict=not soft)  # attempts to find conflict-free renaming order, or exits  # line 1239
    if os.path.exists(encode(newRelPath)):  # line 1240
        exists = [filename[1] for filename in matches if os.path.exists(encode(os.path.join(newRelPath, filename[1]).replace(SLASH, os.sep)))]  # type: _coconut.typing.Sequence[str]  # line 1241
        if exists and not (force or soft):  # line 1242
            Exit("%s files would write over existing files in %s cases. Use --force to execute it anyway" % ("Moving" if relPath != newRelPath else "Renaming", "all" if len(exists) == len(matches) else "some"))  # line 1242
    else:  # line 1243
        os.makedirs(encode(os.path.abspath(newRelPath.replace(SLASH, os.sep))))  # line 1243
    if not soft:  # perform actual renaming  # line 1244
        for (source, target) in matches:  # line 1245
            try:  # line 1246
                shutil.move(encode(os.path.abspath(os.path.join(relPath, source).replace(SLASH, os.sep))), encode(os.path.abspath(os.path.join(newRelPath, target).replace(SLASH, os.sep))))  # line 1246
            except Exception as E:  # one error can lead to another in case of delicate renaming order  # line 1247
                error("Cannot move/rename file '%s' to '%s'" % (source, os.path.join(newRelPath, target)))  # one error can lead to another in case of delicate renaming order  # line 1247
    patterns[patterns.index(pattern)] = newPattern  # line 1248
    m.saveBranches(m._extractRemotesFromArguments(options))  # line 1249

def parse(vcs: 'str', cwd: 'str', cmd: 'str'):  # line 1251
    ''' Main operation. root is underlying VCS base dir. main() has already chdir'ed into SOS root folder, cwd is original working directory for add, rm, mv. '''  # line 1252
    debug("Parsing command-line arguments...")  # line 1253
    root = os.getcwd()  # line 1254
    try:  # line 1255
        onlys, excps, remotes, noremotes = parseArgumentOptions(cwd, sys.argv)  # extracts folder-relative paths (used in changes, commit, diff, switch, update)  # line 1256
        command = sys.argv[1].strip() if len(sys.argv) > 1 else ""  # line 1257
        arguments = [c.strip() for c in sys.argv[2:] if not ((len(c) == 2 and c.startswith("-")) or (len(c) > 2 and c[1] == "-"))]  # type: List[str]  # line 1258
        options = [c.strip() for c in sys.argv[2:] if ((len(c) == 2 and c.startswith("-")) or (len(c) > 2 and c[1] == "-"))]  # type: List[str]  # options *with* arguments have to be parsed directly from sys.argv inside using functions  # line 1259
        debug("Processing command %r with arguments %r and options %r." % (command, [_ for _ in arguments if _ is not None], options))  # line 1260
        if command[:1] in "amr":  # line 1261
            try:  # line 1262
                relPaths, patterns = unzip([relativize(root, os.path.join(cwd, argument)) for argument in ((["."] if arguments is None else arguments))])  # line 1262
            except:  # line 1263
                command = "ls"  # convert command into ls --patterns  # line 1264
                arguments[0] = None  # convert command into ls --patterns  # line 1264
                options.extend(["--patterns", "--all"])  # convert command into ls --patterns  # line 1264
# Exit("Need one or more file patterns as argument (escape them according to your shell)")
        if command[:1] == "m":  # line 1266
            if len(arguments) < 2:  # line 1267
                Exit("Need a second file pattern argument as target for move command")  # line 1267
            newRelPath, newPattern = relativize(root, os.path.join(cwd, arguments[1]))  # line 1268
        arguments[:] = (arguments + [None] * 3)[:3]  # line 1269
        if command == "raise":  # line 1270
            raise Exception("provoked exception")  # line 1270
        elif command[:1] == "a":  # e.g. addnot  # line 1271
            add(relPaths, patterns, options, negative="n" in command)  # e.g. addnot  # line 1271
        elif command[:1] == "b":  # line 1272
            branch(arguments[0], arguments[1], options)  # line 1272
        elif command[:2] == "ch":  # "changes" (legacy)  # line 1273
            changes(arguments[0], options, onlys, excps, cwd)  # "changes" (legacy)  # line 1273
        elif command[:2] == "ci":  # line 1274
            commit(arguments[0], options, onlys, excps)  # line 1274
        elif command[:3] == "com":  # line 1275
            commit(arguments[0], options, onlys, excps)  # line 1275
        elif command[:3] == 'con':  # line 1276
            config(arguments, options)  # line 1276
        elif command[:2] == "de":  # line 1277
            destroy((lambda _coconut_none_coalesce_item: "" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(arguments[0]), options)  # line 1277
        elif command[:2] == "di":  # TODO no consistent handling of single dash/characters argument-options  # line 1278
            diff((lambda _coconut_none_coalesce_item: "/" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(arguments[2 if arguments[0] == '-n' else 0]), options, onlys, excps)  # TODO no consistent handling of single dash/characters argument-options  # line 1278
        elif command[:2] == "du":  # line 1279
            dump((lambda _coconut_none_coalesce_item: "" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(arguments[0]), options)  # line 1279
        elif command[:1] == "h":  # line 1280
            usage.usage(arguments[0], verbose=verbose)  # line 1280
        elif command[:2] == "lo":  # line 1281
            log(options, cwd)  # line 1281
        elif command[:2] == "li":  # line 1282
            ls(os.path.relpath((lambda _coconut_none_coalesce_item: cwd if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(arguments[0]), root), options)  # line 1282
        elif command[:2] == "ls":  # line 1283
        ...
        elif command[:1] == "m":  # e.g. mvnot  # line 1284
            move(relPaths[0], patterns[0], newRelPath, newPattern, options, negative="n" in command)  # e.g. mvnot  # line 1284
        elif command[:2] == "of":  # line 1285
            offline(arguments[0], arguments[1], options, remotes)  # line 1285
        elif command[:2] == "on":  # line 1286
            online(options)  # line 1286
        elif command[:1] == "p":  # line 1287
            publish(arguments[0], cmd, options, onlys, excps)  # line 1287
        elif command[:1] == "r":  # e.g. rmnot  # line 1288
            remove(relPaths, patterns, options, negative="n" in command)  # e.g. rmnot  # line 1288
        elif command[:2] == "st":  # line 1289
            status(arguments[0], vcs, cmd, options, onlys, excps)  # line 1289
        elif command[:2] == "sw":  # line 1290
            switch((lambda _coconut_none_coalesce_item: "" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(arguments[0]), options, onlys, excps, cwd)  # line 1290
        elif command[:1] == "u":  # line 1291
            update((lambda _coconut_none_coalesce_item: "" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(arguments[0]), options, onlys, excps)  # line 1291
        elif command[:1] == "v":  # line 1292
            usage.usage(arguments[0], version=True)  # line 1292
        else:  # line 1293
            Exit("Unknown command '%s'" % command)  # line 1293
        Exit(code=0)  # regular exit  # line 1294
    except (Exception, RuntimeError) as E:  # line 1295
        Exit("An internal error occurred in SOS\nPlease report above message to the project maintainer at  https://github.com/ArneBachmann/sos/issues  via 'New Issue'.\nPlease state your installed version via 'sos version', and what you were doing.", exception=E)  # line 1296

def main():  # line 1298
    global debug, info, warn, error  # to modify logger  # line 1299
    logging.basicConfig(level=level, stream=sys.stderr, format=("%(asctime)-23s %(levelname)-8s %(name)s:%(lineno)d | %(message)s" if '--log' in sys.argv else "%(message)s"))  # line 1300
    _log = Logger(logging.getLogger(__name__))  # line 1301
    debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 1301
    for option in (o for o in ['--log', '--debug', '--verbose', '-v', '--sos', '--vcs'] if o in sys.argv):  # clean up program arguments  # line 1302
        sys.argv.remove(option)  # clean up program arguments  # line 1302
    if '--help' in sys.argv or len(sys.argv) < 2:  # line 1303
        usage.usage(sys.argv[sys.argv.index('--help') + 1] if '--help' in sys.argv and len(sys.argv) > sys.argv.index('--help') + 1 else None, verbose=verbose)  # line 1303
    command = sys.argv[1] if len(sys.argv) > 1 else None  # type: _coconut.typing.Optional[str]  # line 1304
    root, vcs, cmd = findSosVcsBase()  # root is None if no .sos folder exists up the folder tree (=still working online); vcs is checkout/repo root folder; cmd is the VCS base command  # line 1305
    debug("Detected SOS root folder: %s" % (("-" if root is None else root)))  # line 1306
    debug("Detected VCS root folder: %s" % (("-" if vcs is None else vcs)))  # line 1307
    defaults["defaultbranch"] = (lambda _coconut_none_coalesce_item: "default" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(vcsBranches.get(cmd, vcsBranches[SVN]))  # sets dynamic default with SVN fallback  # line 1308
    defaults["useChangesCommand"] = cmd == "fossil"  # sets dynamic default with SVN fallback  # line 1309
    if (not force_vcs or force_sos) and (root is not None or (("" if command is None else command))[:2] == "of" or (("_" if command is None else command))[:1] in "hv"):  # in offline mode or just going offline  # line 1310
        cwd = os.getcwd()  # line 1311
        os.chdir(cwd if command[:2] == "of" else (cwd if root is None else root))  # line 1312
        parse(vcs, cwd, cmd)  # line 1313
    elif force_vcs or cmd is not None:  # online mode - delegate to VCS  # line 1314
        info("%s: Running '%s %s'" % (usage.COMMAND.upper(), cmd, " ".join(sys.argv[1:])))  # line 1315
        import subprocess  # only required in this section  # line 1316
        process = subprocess.Popen([cmd] + sys.argv[1:], shell=False, stdin=subprocess.PIPE, stdout=sys.stdout, stderr=sys.stderr)  # line 1317
        inp = ""  # type: str  # line 1318
        while True:  # line 1319
            so, se = process.communicate(input=inp)  # line 1320
            if process.returncode is not None:  # line 1321
                break  # line 1321
            inp = sys.stdin.read()  # line 1322
        if sys.argv[1][:2] == "co" and process.returncode == 0:  # successful commit to underlying VCS - assume now in sync again (but leave meta data folder with potential other feature branches behind until "online")  # line 1323
            if root is None:  # line 1324
                Exit("Cannot determine SOS root folder: Not working offline, thus unable to mark offline repository as synchronized")  # line 1324
            m = Metadata(root)  # type: Metadata  # line 1325
            m.branches[m.branch] = dataCopy(BranchInfo, m.branches[m.branch], inSync=True)  # mark as committed  # line 1326
            m.saveBranches()  # line 1327
    else:  # line 1328
        Exit("No offline repository present, and unable to detect VCS file tree")  # line 1328


# Main part
force_sos = [None] if '--sos' in sys.argv else []  # type: List[None]  # this is a trick allowing to modify the module-level flags from the test suite  # line 1332
force_vcs = [None] if '--vcs' in sys.argv else []  # type: List[None]  # line 1333
level = logging.DEBUG if '--debug' in sys.argv else logging.INFO  # type: int  # line 1334

_log = Logger(logging.getLogger(__name__))  # line 1336
debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 1336

if __name__ == '__main__':  # line 1338
    main()  # line 1338
